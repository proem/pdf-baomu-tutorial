...
- [ ] 章节标题是普通黑色
- [ ] 每行之间用点状虚线分隔
- [ ] **目录章节数与正文章节数一致**——模板默认列出 9 项 (`零~捌`)，多出来的 `toc-item` 整行删掉，否则会显示空白章节像「陆 ☐」「柒 ☐」
...
### 代码块
- [ ] 深色背景（github-dark `#24292e`），多色文字（关键字 / 字符串 / 注释 不同色）
- [ ] 普通 `<pre>` 块不跨页；但 **Claude Code 紫色引导块允许跨页**（指令可能很长）
- [ ] 行内 `<code>` 是浅灰背景 + 红色文字
- [ ] **代码块在 markdown 里显式标语言**（如 ` ```bash`），否则不会被 Shiki 高亮
- [ ] 长命令在源 markdown 里手动换行，避免 PDF 里只看到截断的一段（Shiki 会自动 wrap，但 wrap 位置不一定语义最佳）
